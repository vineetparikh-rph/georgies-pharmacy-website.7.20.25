...
export default function HealthTipsSidebar({
    ...
    return (
      <Card key={tip.id} className="mb-4 border-l-4 border-l-primary">
        ...
        <CardContent className="pt-0">
          ...
          {tip.source && <p className="text-xs text-slate-400 mt-2">Source: {tip.source}</p>}
        </CardContent>
      </Card>
    );
  };
  ...
  return (
    <div className={`bg-white rounded-lg border border-slate-200 p-6 ${className}`}>
      ...
      <div className="space-y-4">{loading ? renderSkeleton() : tips.map(renderTipContent)}</div>

      {tips.length === 0 && !loading && (
        <div className="text-center py-8">
          <Brain className="h-12 w-12 text-slate-300 mx-auto mb-4" />
          <p className="text-slate-500">No health tips available</p>
          <Button variant="outline" size="sm" onClick={generateHealthTips} className="mt-2">
            Generate Tips
          </Button>
        </div>
      )}
      ...
      <div className="mt-6 pt-4 border-t border-slate-200">
        <p className="text-xs text-slate-500 text-center">
          AI-powered health tips are for informational purposes only. Always consult your healthcare
