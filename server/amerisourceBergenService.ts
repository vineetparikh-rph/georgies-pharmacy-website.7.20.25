...
export class AmerisourceBergenService {
  ...
  constructor() {
    this.baseUrl = process.env.AB_API_BASE_URL || 'https://api.amerisourcebergen.com/v1';
    this.apiKey = process.env.AB_API_KEY || '';
    this.customerId = process.env.AB_CUSTOMER_ID || '';
    this.environment = (process.env.AB_ENVIRONMENT as 'sandbox' | 'production') || 'sandbox';
  }
  ...
  async searchProducts(searchRequest: ABSearchRequest): Promise<ABProduct[]> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`AmerisourceBergen API error: ${response.status} ${response.statusText}`);
      }
  ...
  async placeOrder(orderRequest: ABOrderRequest): Promise<ABOrderResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`AmerisourceBergen order error: ${response.status} ${response.statusText}`);
      }
  ...
  private processProduct(product: any): ABProduct {
    return {
      ...
      contractPrice: product.contractPrice ? parseFloat(product.contractPrice) : undefined,
  ...
  convertToStandardProduct(abProduct: ABProduct): any {
    ...
    return {
      ...
      comparePrice: savings > 0 ? `$${abProduct.unitPrice.toFixed(2)}` : undefined,
      ...
      inStock: abProduct.availability === 'available' && abProduct.inventoryQuantity > 0,
