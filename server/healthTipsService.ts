...
export async function generateHealthTips(
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a helpful pharmacy AI assistant providing evidence-based health tips. Always recommend consulting healthcare providers for medical decisions.",
          },
          {
            role: "user",
            content: prompt,
    ...
    return parsedTips.length > 0
      ? parsedTips
      : getFallbackHealthTips(medications);
  } catch (error) {
...
function createHealthTipsPrompt(
  medications: string[],
  preferences: string[],
): string {
  let prompt =
    "Provide 3-4 personalized health tips in JSON format. Each tip should have: id, title, content, category (medication/wellness/nutrition/exercise/general), and relevance (high/medium/low).";
  ...
  prompt +=
    " Keep tips practical, evidence-based, and under 100 words each. Always recommend consulting healthcare providers.";
...
function parseStructuredText(text: string): HealthTip[] {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      currentTip.category = inferCategory(
...
function inferCategory(
  text: string,
): "medication" | "wellness" | "nutrition" | "exercise" | "general" {
  ...
  const baseTips = [
    {
      ...
      content:
        "Take your medications at the same time each day to maintain consistent levels. Use pill organizers or phone reminders if helpful.",
    ...
    },
    {
      ...
      title: "Stay Hydrated",
      content:
        "Drink 8-10 glasses of water daily. Proper hydration helps your body process medications effectively and supports overall health.",
    ...
    },
    {
      ...
      content:
        "Aim for 30 minutes of moderate exercise most days. Consult your healthcare provider about any restrictions with your current medications.",
    ...
    },
    {
      ...
      content:
        "Eat a balanced diet rich in fruits, vegetables, and whole grains. Some medications work better with food, others on an empty stomach.",
