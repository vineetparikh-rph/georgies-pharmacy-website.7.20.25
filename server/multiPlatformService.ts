...
export class MultiPlatformService {
  ...
  private initializePlatforms() {
    ...
    this.platforms.set("amazon", {
      ...
      enabled: !!(
        process.env.AMAZON_SP_API_ACCESS_KEY && process.env.AMAZON_SELLER_ID
      ),
    });
    ...
    this.platforms.set("uber", {
      ...
      enabled: !!(
        process.env.UBER_DIRECT_API_KEY && process.env.UBER_DIRECT_CUSTOMER_ID
      ),
    });
    ...
    this.platforms.set("instacart", {
      ...
      enabled: !!(
        process.env.INSTACART_API_KEY && process.env.INSTACART_RETAILER_ID
      ),
    });
    ...
    this.platforms.set("doordash", {
      ...
      enabled: !!(
        process.env.DOORDASH_API_KEY && process.env.DOORDASH_BUSINESS_ID
      ),
    });
    ...
    this.platforms.set("postmates", {
      ...
      enabled: !!(
        process.env.POSTMATES_API_KEY && process.env.POSTMATES_CUSTOMER_ID
      ),
    });
  }
      ...
      enabledPlatforms.map(async (platform) => {
        try {
          const platformUpdates = updates.filter(
            (update) =>
              update.platform === "all" ||
              update.platform === platform.name.toLowerCase(),
          );
  ...
  private async syncToDoorDash(
    ...
    if (!response.ok) {
      throw new Error(
        `DoorDash API error: ${response.status} ${response.statusText}`,
      );
    }
  ...
  private async updateAmazonInventory(
    ...
    const response = await fetch(
      `${config.baseUrl}/fba/inventory/v1/summaries`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${config.apiKey}`,
   ...
   */
  convertABProductToPlatformProduct(
    abProduct: any,
    storeLocation: string,
  ): PlatformProduct {
    return {
      ...
      comparePrice:
        abProduct.unitPrice > (abProduct.contractPrice || 0)
          ? abProduct.unitPrice
          : undefined,
   ...
   */
  getPlatformStatus(): {
    [platform: string]: { enabled: boolean; configured: boolean };
  } {
    const status: {
      [platform: string]: { enabled: boolean; configured: boolean };
    } = {};
