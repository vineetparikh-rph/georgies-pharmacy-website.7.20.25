...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/pharmacy-login", (req, res) => {
    res.sendFile(join(__dirname, '../prescription-services/pharmacy_login.html'));
  });
  ...
  app.get("/api/transfer", (req, res) => {
    res.sendFile(join(__dirname, '../prescription-services/transfer_form.html'));
  });

  app.get("/api/vaccine", (req, res) => {
    res.sendFile(join(__dirname, '../prescription-services/vaccine_consent.html'));
  });
  ...
  const { sendFaxEmail, generateRefillPDF, generateTransferPDF, generateVaccinePDF } = await import('./emailService.js');
  
  // Import Shopify service
  const { shopifyService } = await import('./shopifyService.js');
  
  // Import AmerisourceBergen service
  const { amerisourceBergenService } = await import('./amerisourceBergenService.js');
  ...
  app.post("/api/submit-refill", express.urlencoded({ extended: true }), async (req, res) => {
    try {
      const { first_name, last_name, dob, phone, selected_location } = req.body;
...
${medications}`;
      ...
      // Send fax email
      const result = await sendFaxEmail(selected_location.toUpperCase(), emailBody, "Refill Request", pdfPath);
      
      if (result.status === "success") {
        res.redirect(`/fax-confirmation?type=refill&location=${selected_location}`);
      } else {
        res.status(500).json({ success: false, message: result.message });
      }
    } catch (error) {
      console.error('Refill submission error:', error);
      res.status(500).json({ success: false, message: "Failed to process refill request" });
    }
  });

  app.post("/api/submit-transfer", express.urlencoded({ extended: true }), async (req, res) => {
    try {
      const {
        first_name, last_name, dob, phone, selected_location,
        address1, address2, city, state, zip,
        current_pharmacy_name, current_pharmacy_phone, medications,
        other_pharmacy_name, other_pharmacy_phone
      } = req.body;
      
      const fullName = `${first_name} ${last_name}`;
      const fullAddress = `${address1}, ${address2 || ''}, ${city}, ${state} ${zip}`.replace(', ,', ',');
...
${medicationList}`;
      ...
      if (!locationCode) {
        return res.status(400).json({ success: false, message: "Please select a pharmacy location" });
      }
      const result = await sendFaxEmail(locationCode.toUpperCase(), emailBody, "Transfer Request", pdfPath);
      
      if (result.status === "success") {
        res.redirect(`/fax-confirmation?type=transfer&location=${locationCode}`);
      } else {
        res.status(500).json({ success: false, message: result.message });
      }
    } catch (error) {
      console.error('Transfer submission error:', error);
      res.status(500).json({ success: false, message: "Failed to process transfer request" });
    }
  });

  app.post("/api/submit-vaccine", express.urlencoded({ extended: true }), async (req, res) => {
    try {
      ...
      // Send fax email
      const result = await sendFaxEmail(location.toUpperCase(), emailBody, "Vaccine Consent Form", pdfPath);
      ...
      res.status(500).json({ success: false, message: "Failed to process vaccine consent form" });
    }
  });
  ...
  app.get("/api/otc-products", async (req, res) => {
    try {
      ...
      if (amerisourceBergenService.isConfigured()) {
        const abProducts = await amerisourceBergenService.getFeaturedProducts(100);
        const standardProducts = abProducts.map(product => {
          const converted = amerisourceBergenService.convertToStandardProduct(product);
          return {
            ...converted,
            storeLocation: store,
      ...
      const mockProducts = [
          {
            ...
            reviewCount: 124,
            inStock: true,
            shopifyUrl: "https://georgiespharmacy.com/products/tylenol-extra-strength"
          },
          {
            id: "2",
            ...
            price: "$15.99",
            image: "https://images.unsplash.com/photo-1556909114-0a5e16e77ff6?w=400",
            category: "vitamins",
            brand: "Nature Made",
            rating: 4.8,
            reviewCount: 89,
          ...
          },
          {
            ...
            description: "Flexible fabric bandages that move with you for superior comfort",
            price: "$4.99",
            image: "https://images.unsplash.com/photo-1603398938093-6b77de70db2c?w=400",
            category: "first-aid",
            brand: "Band-Aid",
            rating: 4.3,
            reviewCount: 67,
            inStock: true,
            shopifyUrl: "https://georgiespharmacy.com/products/band-aid-flexible-fabric"
          },
          {
            id: "4",
          ...
          },
          {
            ...
            reviewCount: 93,
            inStock: true,
            shopifyUrl: "https://georgiespharmacy.com/products/tums-ultra-strength"
          },
          {
            id: "6",
            title: "Robitussin DM Cough Syrup",
            description: "Controls cough and thins mucus for cold and flu symptoms",
            price: "$9.99",
            image: "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=400",
            category: "cold-flu",
            brand: "Robitussin",
        ...
        return res.json({ 
          ...
          source: "Demo",
          message: "Using demo data. Configure AmerisourceBergen or Shopify credentials for live products."
        });
    } catch (error) {
  ...
  app.get("/api/otc-products/search", async (req, res) => {
    try {
      ...
      if (amerisourceBergenService.isConfigured()) {
        const abProducts = await amerisourceBergenService.searchProducts({ query });
        const standardProducts = abProducts.map(product => {
          const converted = amerisourceBergenService.convertToStandardProduct(product);
          return {
            ...converted,
      ...
      return res.json({ 
        products: [],
        message: "Search not available. Configure AmerisourceBergen or Shopify credentials."
      });
    } catch (error) {
  ...
  app.get("/api/otc-products/category/:category", async (req, res) => {
    try {
      ...
      if (!shopifyService.isConfigured()) {
        return res.json({ 
          products: [],
          message: "Category filtering not available in demo mode. Configure Shopify credentials."
        });
      }
  ...
  app.post("/api/otc-products/sync-platforms", async (req, res) => {
    try {
      ...
      const abProduct = await amerisourceBergenService.getProductDetails(productId);
      if (!abProduct) {
        return res.status(404).json({ message: "Product not found" });
      }

      // Convert to platform format
      const platformProduct = multiPlatformService.convertABProductToPlatformProduct(abProduct, storeLocation);
      
      // Sync to all enabled platforms
      const results = await multiPlatformService.syncProductToAllPlatforms(platformProduct, storeLocation);
  ...
  app.post("/api/platforms/inventory-update", async (req, res) => {
    try {
      ...
      const results = await multiPlatformService.updateInventoryAllPlatforms(updates);
  ...
  app.get("/api/otc-products/details/:productId", async (req, res) => {
    try {
      ...
      if (amerisourceBergenService.isConfigured()) {
        const abProduct = await amerisourceBergenService.getProductDetails(productId);
        if (abProduct) {
          const standardProduct = amerisourceBergenService.convertToStandardProduct(abProduct);
          return res.json({
            ...standardProduct,
            storeLocation: store,
  ...
  app.post("/api/otc-products/order", async (req, res) => {
    try {
      const { productId, store, quantity, deliveryMethod, customerInfo, shippingAddress, totalAmount } = req.body;
      
      // Generate order ID
      const orderId = `ORD-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      
      // Get product details
      const product = await amerisourceBergenService.getProductDetails(productId);
      if (!product) {
        return res.status(404).json({ message: "Product not found" });
      }
      ...
      const orderData = {
        ...
        items: [{
          ...
          quantity: quantity,
          unitPrice: parseFloat(product.unitPrice || product.contractPrice || '0')
        }],
      ...
      res.json({
        ...
        status: 'confirmed',
        estimatedDelivery: deliveryMethod === 'delivery' ? '2-3 business days' : 'Ready for pickup',
