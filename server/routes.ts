...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const { sendFaxEmail, generateRefillPDF, generateTransferPDF, generateVaccinePDF } = await import(
    './emailService.js'
  );
...
${medicationList}`;
      ...
      if (!locationCode) {
        return res
          .status(400)
          .json({ success: false, message: 'Please select a pharmacy location' });
      }
      const result = await sendFaxEmail(
  ...
  app.post('/api/otc-products/sync-platforms', async (req, res) => {
    try {
      ...
      const platformProduct = multiPlatformService.convertABProductToPlatformProduct(
        abProduct,
        storeLocation
      );
