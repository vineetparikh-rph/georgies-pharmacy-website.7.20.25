...
export class MemStorage implements IStorage {
  ...
  private seedData() {
    ...
    locations.forEach((location) =>
      this.pharmacyLocations.set(location.id, location),
    );
  ...
  async createRefillRequest(
    insertRequest: InsertRefillRequest,
  ...
  async getRefillRequestsByPatientId(
    patientId: number,
  ): Promise<RefillRequest[]> {
    return Array.from(this.refillRequests.values()).filter(
      (r) => r.patientId === patientId,
    );
  }

  async createTransferRequest(
    insertRequest: InsertTransferRequest,
  ): Promise<TransferRequest> {
    const id = this.currentId++;
  ...
  async getTransferRequestsByPatientId(
    patientId: number,
  ): Promise<TransferRequest[]> {
    return Array.from(this.transferRequests.values()).filter(
      (r) => r.patientId === patientId,
    );
  }
